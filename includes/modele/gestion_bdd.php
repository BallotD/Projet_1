...
function GetAllRequests($year) // obtenir toutes les requêtes de demie journée d'un mois donné en parametre 
{
	...
	$lesdemandes = $bdd->query(' select ID_request, date, id_utilisateur, year(date) as annee,  day(date) as jour, month(date) as mois, accepte   from intervention where year(date) = ' . $year );
...
function GetRequestbyID($ID) // obtenir toutes les requêtes de demie journée d'un mois donné en parametre 
{
	$connect = new connexion();
	$bdd = $connect->getInstance();
	$lesdemandes = $bdd->query(' select *   from intervention where ID_request = ' . $ID);
...
function reponseDemieJournee($id, $reponse, $commentaire)//repondre à une demie journée
{
	$connect = new connexion();
	$bdd = $connect->getInstance();
	$bdd->exec('update intervention set accepte = '.  $reponse .', commentaire = "'.  $commentaire . '" where ID_request = '.$id.'  ;');
...
function deleteUser($user)//supprimer un utilisateur et ses journées de congéaddNewUser
{
	$connect = new connexion();
	$bdd = $connect->getInstance();	
	if ($bdd->exec('delete from intervention where id_utilisateur = '.$user.';
...
function deleteRequests($id)
{
	$connect = new connexion();
	$bdd = $connect->getInstance();	
	$bdd->exec('delete from intervention where id_utilisateur = '.$id.';');
}
...
function deleteRequestbyID($id)
{
	$connect = new connexion();
	$bdd = $connect->getInstance();	
	$bdd->exec('delete from intervention where ID_request = '.$id.';');
}
