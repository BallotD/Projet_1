						...
						if ($dddate->format('d') == $lerepos['jour'] and $lerepos['mois'] == $dddate->format('m') and $lerepos['annee'] == $dddate->format('Y') )
						{
							if ($_SESSION['droits'] == '2')
							{
								...
								$curUser = getUserbyID($lerepos['id_utilisateur']);
									if(is_null($lerepos['accepte']))
									{
										$string .='<span style="color:darkgoldenrod;cursor:pointer;" onclick="raiseRequestAnwer('.$lerepos['ID_request'].')">' .substr($curUser['prenom'], 0, 1).'. '.$curUser['nom'].'</span><br>';
									}
									else if ($lerepos['accepte'] == '0')
									{
										$string .='<span style="color:orange;cursor:pointer;" onclick="raiseRequestAnwer('.$lerepos['ID_request'].')">' .substr($curUser['prenom'], 0, 1).'. '.$curUser['nom'].'</span><br>';
									}
									else if ($lerepos['accepte'] == '1')
									{
										$string .='<span style="color:green;cursor:pointer;" onclick="raiseRequestAnwer('.$lerepos['ID_request'].')">' .substr($curUser['prenom'], 0, 1).'. '.$curUser['nom'].'</span><br>';
									}
								}
